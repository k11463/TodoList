<template>
    <div class="bg">
        <div class="container">
            <div class="newTask">
                <input type="text" placeholder="請輸入新清單標題...按Enter新增" @keyup.enter="createdList" v-model="newTitle">
            </div>
...
export default {
    mounted() {
        for (let i = 0; i < this.$store.state.items.length; i++){
            this.filterdItem = this.$store.state.items.filter(el => (!el.checked && !el.star));
        }
        //console.log(a);
    },
    data(){
    ...
    methods: {
        createdList(){
            this.$store.commit('addNewList', this.newTitle);
            for (let i = 0; i < this.$store.state.items.length; i++){
                this.filterdItem = this.$store.state.items.filter(el => (!el.checked && !el.star));
            }
        },
